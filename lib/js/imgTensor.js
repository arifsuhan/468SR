...
class imgTensor 
{
  ...
  draw()
  {
    this.canvas.width = this.inputImg.width/4;
    this.canvas.height = this.inputImg.height/4;
  ...
  sharpen(matrix)
  {
    ...
    // var addPad1 = matrix[0].length+ 2*1;
    ...
    var r = conv2dTensor(matrix[0],[3,3],[0], 1, filter, [0]);
    var g = conv2dTensor(matrix[1],[3,3],[0], 1, filter, [0]);
    var b = conv2dTensor(matrix[2],[3,3],[0], 1, filter, [0]);
    var a = conv2dTensor(matrix[3],[3,3],[0], 1, filter, [0]);
  ...
  applyKernel()
  {
    ...
    const pixels= this.imageObj.data;
